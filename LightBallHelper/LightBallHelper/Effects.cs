...
namespace LightBallHelper {
    ...
    abstract class Effects {
        ...
        public enum EffectsNames { EffectHSL, EffectColorTransition, EffectSparkAsync, EffectSparkSync, EffectMeteorAsync, EffectMeteorSync, EffectBounceSync, EffectBounceAsync, End };
        ...
        public static void ExportTasks(string filename, List<Effects> effects) {
            for ( int i = 0; i < numOfBall; i++ ) {
                ...
                StreamWriter sw = new StreamWriter(filename + i.ToString() + ".h");
                const string header = "#ifndef TASK_H  \n#define TASK_H  \n#include \"../Constants.h\"  \n#include \"../Essentials.h\"  \n#define CMD_PARAMETER_LENGTH 14  \nconst byte cmd_buffer[][CMD_PARAMETER_LENGTH] = {";
                const string rear = "};  \n#endif";
    ...
    class EffectHSL : Effects {
        ...
        public override string GetTasks(int ballID) {
            ...
            keypoints.ForEach((key) =>
            {
                cmd += "{"+effectCmdName+", " + key.getBasicInfo(360*ballID/numOfBall) + ", 0x00, 0x00, 0x00, 0x00},\n";
            });
            return cmd;
        }
    }
    ...
    abstract class EffectBounce : Effects {
        protected override void paraInit() {
            base.autoGenMode = AutoGenMode.enable;
            base.para_label = new string[] { "Count", "BounceSpeed", "Duty", "ColorTran", "BounceCount", "Phase" };
            base.para_unit = new string[] { "times", "sec", "%", "sec", "sec", "deg" };
            base.para_value = new string[] { "1", "5", "100", "0", "5", "0" };
            para_len = para_value.Length;
        }
    ...
    class EffectBounceAsync : EffectBounce {
        public EffectBounceAsync(TimeSpan start_time)
            : base(start_time, EffectsNames.EffectBounceAsync) {
            base.effectCmdName = "LM_SET_BOUNCE_ASYNC";
        }
        public EffectBounceAsync(string[] strs)
            : base(strs, EffectsNames.EffectBounceAsync) {
            base.effectCmdName = "LM_SET_BOUNCE_ASYNC";
        }
    }
    class EffectBounceSync : EffectBounce {
        public EffectBounceSync(TimeSpan start_time)
            : base(start_time, EffectsNames.EffectBounceSync) {
            base.effectCmdName = "LM_SET_BOUNCE_SYNC";
        }
        public EffectBounceSync(string[] strs)
            : base(strs, EffectsNames.EffectBounceSync) {
            base.effectCmdName = "LM_SET_BOUNCE_SYNC";
        }
    }
}
