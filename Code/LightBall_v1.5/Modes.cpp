...
void set_hsl_spark_async(uint24 index, uint16 duration, 
    ...
    float n_l = ((float)(l/255.0)+getFloatFromInt(brightTrans)*(index));//-0.5*pow(2.7, getFloatFromInt(brightTrans)*index/1000));
    n_l = (n_l > 1) ? 1 : n_l;
    n_l = (n_l <= 0) ? 0 : n_l;
...
void set_hsl_spark_sync(uint24 index, uint16 duration, 
                         ...
                         uint8 count, uint8 duty){
    uint16 size = duration/count;       //Frame size
    int n_h = (int)((float)(h/0.7f)+colorTrans*(index/size))%360;
    if (100*(index%size)/size < duty){  //In range of duty
      //int n_h = (int)((float)(h/0.7f)+colorTrans*(index%size)/1000)%360;
      float n_l = (float)(l/255.0)+getFloatFromInt(brightTrans)*(index%size);
      set_hsl(n_h, s/255.0, n_l);  // unit: H/1000s, L/1000s
...
/* The starting color of each cycle is the same.
   And the brights fade automatically.
   * colorTran is the delta color between meteor 
   * meteorTran is the percentage of color tran in each meteor(0~255)
   */
void set_hsl_meteor_sync(uint24 index, uint16 duration, 
    ...
    if (255*(index%size)/size < duty){  //In range of duty
      float dt = (index%size)*255.0/duty/size;
      set_hsl((n_h + meteorTrans*(index%size)*10/7/size)%360, s/255.0, 0.5*pow(2.7, -5*dt));  // unit: H/1000s, L/1000s
    }
    else
      set_rgb(0, 0, 0);
...
void set_bounce_async(uint24 index, uint16 duration, 
                         uint8 h, uint8 s, uint8 l, 
                         uint8 colorTrans, 
                         uint8 count, uint8 bounceCount, uint8 bounceSpeed){
    uint16 size = duration/count;       //Frame size
    int n_h = (int)((float)(h/0.7f)+colorTrans*(index/size)*size/1000+ index%size*360/size)%360;
    uint16 bounceSize = duration/bounceCount;
    uint8 duty = 255*(sin((float)3.14*(index%bounceSize)/bounceSize));
    if (255*(index%size)/size < duty){  //In range of duty
      set_hsl(n_h, s/255.0, l/255.0);  // unit: H/1000s, L/1000s
    }
    else
      set_rgb(0, 0, 0);
}
